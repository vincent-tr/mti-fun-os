...
use alloc::boxed::Box;
use libruntime::{kobject, state::messages::StateServerError};
use log::error;

/// Internal error type with context
pub struct InternalError {
    target: StateServerError,
    context: &'static str,
...
impl InternalError {
    pub fn invalid_argument(context: &'static str) -> Self {
        Self {
            target: StateServerError::InvalidArgument,
            context,
            source: None,
        }
    }
    ...
    pub fn runtime_error(context: &'static str) -> Self {
        Self {
            target: StateServerError::RuntimeError,
            context,
            source: None,
        }
    }
    ...
    /// Log and convert to StateServerError
    pub fn into_server_error(self) -> StateServerError {
