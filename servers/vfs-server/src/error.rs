...
use alloc::boxed::Box;
use libruntime::{kobject, vfs::messages::VfsServerError};
use log::error;

/// Internal error type with context
pub struct InternalError {
    target: VfsServerError,
    context: &'static str,
...
impl InternalError {
    pub fn invalid_argument(context: &'static str) -> Self {
        Self {
            target: VfsServerError::InvalidArgument,
            context,
            source: None,
        }
    }
    ...
    pub fn runtime_error(context: &'static str) -> Self {
        Self {
            target: VfsServerError::RuntimeError,
            context,
            source: None,
        }
    }
    ...
    pub fn buffer_too_small(context: &'static str) -> Self {
        Self {
            target: VfsServerError::BufferTooSmall,
            context,
            source: None,
        }
    }
...
impl Into<VfsServerError> for InternalError {
    fn into(self) -> VfsServerError {
