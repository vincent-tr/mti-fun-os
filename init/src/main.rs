...
fn start_core_servers(info: &InitInfo) {
    ...
    helper.start_server("servers/fs/memfs-server", "file.fs.mem");
    helper.start_server("servers/fs/archivefs-server", "file.fs.archive");
}
...
fn setup_initial_filesystem(info: &InitInfo) {
    ...
    file::mount("/", "file.fs.mem", args.as_slice()).expect("Could not mount memfs");
    ...
    file::mount("/mnt/archive", "file.fs.archive", info.archive_buffer())
...
fn setup_net() {
    ...
    let net_client = net::iface::Client::new();
    net_client
