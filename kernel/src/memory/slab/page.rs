...
pub struct ObjectPage<'a> {
    ...
    #[allow(dead_code)]
    data: [u8; PAGE_SIZE - OBJECT_PAGE_METADATA_OVERHEAD],
...
impl<'a, T: AllocablePage> PageList<'a, T> {
    ...
    pub fn iter_mut<'b: 'a>(&mut self) -> ObjectPageIterMut<'b, T> {
    ...
    /// Removes `slab_page` from the list.
    pub fn pop<'b>(&'b mut self) -> Option<&'a mut T> {
        match self.head {
            None => None,
            Some(ref mut head) => {
    ...
    pub fn contains(&mut self, s: *const T) -> bool {
    ...
    /// Test if there is at least one item in the list
...
pub struct ObjectPageIterMut<'a, P: AllocablePage> {
    head: Rawlink<P>,
