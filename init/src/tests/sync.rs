...
pub fn test_futex() {
    ...
    info!("Test 1: Basic wait/wake");
    {
        ...
        time::sleep(Duration::milliseconds(100));
...
pub fn test_rwlock() {
    ...
    info!("Test 2: Multiple concurrent readers");
    {
        ...
        for i in 0..NUM_READERS {
            ...
            let reader = move || {
                let guard = lock.read();
                assert_eq!(*guard, 42);
                time::sleep(Duration::milliseconds(10));
    ...
    info!("Test 3: Writer excludes readers");
    {
        ...
        *guard = 100;
        time::sleep(Duration::milliseconds(50));
        drop(guard);
    ...
    info!("Test 4: Reader/writer alternation");
    {
        ...
        let writer = move || {
            for i in 0..5 {
                ...
                *guard = i * 10;
                time::sleep(Duration::milliseconds(5));
            }
            let mut msg = unsafe { kobject::Message::new::<u32>(&0, KHandles::new().into()) };
        ...
        for _ in 0..3 {
            ...
            r#async::spawn(async move {
                for _ in 0..5 {
                    ...
                    time::sleep(Duration::milliseconds(3));
                }
            });
        }
